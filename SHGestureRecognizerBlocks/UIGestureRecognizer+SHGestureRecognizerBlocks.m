...
-(instancetype)SH_initWithBlock:(SHGestureRecognizerBlock)theBlock; {
  return [self initWithBlock:theBlock];
}

+(instancetype)SH_recognizerWithBlock:(SHGestureRecognizerBlock)theBlock; {
  return [[[self class] alloc] SH_initWithBlock:theBlock];
}
...
-(void)performBlockAction:(UIGestureRecognizer *)theGestureRecognizer {
  ...
  [theGestureRecognizer.SH_blocks enumerateObjectsUsingBlock:^(SHGestureRecognizerBlock block, NSUInteger _, BOOL *__) {
